...
// Format the specified FILETIME to output in a human readable format,
// without using the C run time.
static void FormatTimeForFilename(LPTSTR output, FILETIME TimeToPrint)
{
	output[0] = _T('\0');
	WORD Date, Time;
	if (FileTimeToLocalFileTime(&TimeToPrint, &TimeToPrint) &&
		FileTimeToDosDateTime(&TimeToPrint, &Date, &Time))
	{
		wsprintf(output, _T("%d%d%d_%02d_%02d_%02d"),
			(Date / 32) & 15, Date & 31, (Date / 512) + 1980,
			(Time >> 11), (Time >> 5) & 0x3F, (Time & 0x1F) * 2);
	}
}
...
{	
	...
	TCHAR szCrashLogFileName[MAX_PATH*2];
	ZeroMemory(szCrashLogFileName, sizeof(szCrashLogFileName));
	...
	GetSystemTimeAsFileTime(&CurrentTime);
	TCHAR szTimeBuffer[100];
	FormatTimeForFilename(szTimeBuffer, CurrentTime);
	
	wsprintf(szCrashLogFileName, "C:\\temp\\InstallationManager_CRASHLOG_%s.log", szTimeBuffer);

	HANDLE hLogFile = CreateFile(szCrashLogFileName, GENERIC_WRITE, 0, 0,
