...
CSMTPSocket::CSMTPSocket()
{
	...
	if (err != 0)
	{
		CLogger::LogMessage("Error calling WSAStartup()");
	}
}
...
BOOL CSMTPAttachment::Attach(const CString& sFilename)
{
  ...
  {
	CString strTemp;strTemp.Format(_T("Failed to get the status for file %s, probably does not exist\n"), sFilename);
	CLogger::LogMessage(strTemp);
    return FALSE;
  }
  ...
  {
    CString strTemp;strTemp.Format(_T("Failed to open file to be attached\n"));
	CLogger::LogMessage(strTemp);
    return FALSE;
  }

  //read in the contents of the input file
  char* pszIn = new char[(unsigned int)fs.m_size];
  infile.Read(pszIn, (UINT)fs.m_size);

  //allocate the encoded buffer
  int nOutSize = Base64BufferSize((int)fs.m_size);
  m_pszEncoded = new char[nOutSize];

  //Do the encoding
  EncodeBase64(pszIn, (int)fs.m_size, m_pszEncoded, nOutSize, &m_nEncodedSize);
...
BOOL CSMTPMessage::AddMultipleRecipients(const CString& sRecipients, RECIPIENT_TYPE RecipientType)
{
	...
	for (int pos=0, start=0; pos<=length; pos++)
	{
      ...
      CSMTPAddress To;
          ...
          CString strTemp;strTemp.Format(_T("An error occurred while parsing the recipients string\n"));
		  CLogger::LogMessage(strTemp);
					return FALSE;
				}
...
BOOL CSMTPConnection::Connect(LPCTSTR pszHostName, int nPort)
{
  ...
  if (!m_SMTP.Create())
  {
    CString strTemp;strTemp.Format(_T("Failed to create client socket, GetLastError returns: %d\n"), GetLastError());
	CLogger::LogMessage(strTemp);
    return FALSE;
  }
  ...
  {
    CString strTemp;strTemp.Format(_T("Could not connect to the SMTP server %s on port %d, GetLastError returns: %d\n"), pszHostName, nPort, GetLastError());
	CLogger::LogMessage(strTemp);
    return FALSE;
  }
  else
  {
    ...
    {
      CString strTemp;strTemp.Format(_T("An unexpected SMTP login response was received\n"));
	  CLogger::LogMessage(strTemp);
      Disconnect();
      return FALSE;
    }
    ...
    {
      Disconnect();
      CString strTemp;strTemp.Format(_T("An unexpected error occurred while sending the HELO command\n"));
	  CLogger::LogMessage(strTemp);
      return FALSE;
    }
    ...
    {
      Disconnect();
      CString strTemp;strTemp.Format(_T("An unexpected HELO response was received\n"));
	  CLogger::LogMessage(strTemp);
      return FALSE;
    } 
...
BOOL CSMTPConnection::Disconnect()
{
  ...
  if (m_bConnected)
  {
	...
	{
		CString strTemp;strTemp.Format(_T("Failed in call to send QUIT command, GetLastError returns: %d\n"), GetLastError());
		CLogger::LogMessage(strTemp);
	}
    ...
    if (!bSuccess)
    {
      ...
      CString strTemp;strTemp.Format(_T("An unexpected QUIT response was received\n"));
	  CLogger::LogMessage(strTemp);
    }
  ...
  {
	CString strTemp;strTemp.Format(_T("Already disconnected from SMTP server, doing nothing\n"));
	CLogger::LogMessage(strTemp);
  }
...
BOOL CSMTPConnection::SendMessage(CSMTPMessage& Message)
{
	...
	{
		CString strTemp;strTemp.Format(_T("Failed in call to send MAIL command, GetLastError returns: %d\n"), GetLastError());
		CLogger::LogMessage(strTemp);
		return FALSE;
	}
	...
	{
		SetLastError(ERROR_BAD_COMMAND);
		CString strTemp;strTemp.Format(_T("An unexpected MAIL response was received\n"));
		CLogger::LogMessage(strTemp);
		return FALSE;
	} 
	...
	{
		SetLastError(ERROR_BAD_COMMAND);
		CString strTemp;strTemp.Format(_T("An unexpected DATA response was received\n"));
		CLogger::LogMessage(strTemp);
		return FALSE;
	} 
	...
	{
		CString strTemp;strTemp.Format(_T("Failed in call to send the header, GetLastError returns: %d\n"), GetLastError());
		CLogger::LogMessage(strTemp);
		return FALSE;
	}
	...
	if (Message.m_Attachments.GetSize())
	{
		...
		{
		CString strTemp;strTemp.Format(_T("Failed in call to send the body header, GetLastError returns: %d\n"), GetLastError());
		CLogger::LogMessage(strTemp);
		return FALSE;
		}
	}
  ...
  for (int i=0; i<Message.m_Attachments.GetSize(); i++)
  {
    CSMTPAttachment* pAttachment = Message.m_Attachments.GetAt(i);
		...
		{
			CString strTemp;strTemp.Format(_T("Failed in call to send Mime attachment header, GetLastError returns: %d\n"), GetLastError());
			CLogger::LogMessage(strTemp);
			return FALSE;
		}
    ...
    {
      CString strTemp;strTemp.Format(_T("Failed in call to send the attachment, GetLastError returns: %d\n"), GetLastError());
	  CLogger::LogMessage(strTemp);
      return FALSE;
    }
  }
  ...
  {
    CString strTemp;strTemp.Format(_T("Failed in call to send end of message indicator, GetLastError returns: %d\n"), GetLastError());
	CLogger::LogMessage(strTemp);
    return FALSE;
  }
  ...
  {
    SetLastError(ERROR_BAD_COMMAND);
    CString strTemp;strTemp.Format(_T("An unexpected end of message response was received\n"));
	CLogger::LogMessage(strTemp);
    return FALSE;
  } 
...
BOOL CSMTPConnection::SendRCPTForRecipient(CSMTPAddress& recipient)
{
  ...
  {
    CString strTemp;strTemp.Format(_T("Failed in call to send RCPT command, GetLastError returns: %d\n"), GetLastError());
	CLogger::LogMessage(strTemp);
    return FALSE;
  }
  ...
  {
    SetLastError(ERROR_BAD_COMMAND);
    CString strTemp;strTemp.Format(_T("An unexpected RCPT response was received\n"));
	CLogger::LogMessage(strTemp);
    return FALSE;
  } 
...
bool CSMTPMail::Send(CString strTo, CString strFrom, CString strSubject, CString strBody, CString strServer)
{
	bool bOK = false;
	...
	CLogger::LogMessage("Sending Email");

	if ((strServer.GetLength() > 0) && (strTo.GetLength() > 0))
	{
		if (smtp.Connect(strServer))
		{
			m.AddMultipleRecipients(strTo, CSMTPMessage::TO);
			CSMTPAddress from = CSMTPAddress(strFrom);
			m.AddRecipient(from, CSMTPMessage::CC);   // send copy to current user
			m.AddBody(strBody);
			m.m_From = strFrom;
			m.m_sSubject = strSubject;

			bOK = (smtp.SendMessage(m) == TRUE);
		}
		else
		{
			m_strLastError = "Could not connect to the SMTP server.";
		}
	}
...
CString CSMTPMail::GetLastError()
{
