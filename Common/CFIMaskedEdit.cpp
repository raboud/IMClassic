...
The purpose of this class is to provide an override of the WindowsProc in the CCFIMaskedEdit class.
There are two versions of the CCFIMaskedEdit class - one in the UT and one in the UG.  They are not
the same and you cannot use the grid version in place of where you need the toolbox version.  Since
they are named the same, the two classes cannot be in the same project.

*/

// Copyright © Dundas Software Ltd. 1997 - 1998, All Rights Reserved

// CFIMaskedEdit.cpp : implementation file
//
/////////////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "CFIMaskedEdit.h"    // CCFIMaskedEdit
...
//		bIsInputData=TRUE ;
//		break;
//	// These are separators and literals.  They are not input types. 
...
IMPLEMENT_DYNAMIC(CCFIMaskedEdit, CEdit)

CCFIMaskedEdit::CCFIMaskedEdit(LPCTSTR pszMask/*=_T("")*/) : 
	m_bInsertMode(TRUE),
...
CCFIMaskedEdit::~CCFIMaskedEdit()
{
	DeleteContents();
}


BEGIN_MESSAGE_MAP(CCFIMaskedEdit, CEdit)
	//{{AFX_MSG_MAP(CCFIMaskedEdit)
	ON_WM_KEYDOWN()
	ON_WM_CHAR()
	ON_WM_SETFOCUS()
...
/////////////////////////////////////////////////////////////////////////////
// CCFIMaskedEdit operations

void CCFIMaskedEdit::DeleteContents()
{
...
CString CCFIMaskedEdit::GetMask() const
{
	CString csMask;
	CMaskData* pobjData=NULL;
	for(POSITION pos=m_listData.GetHeadPosition(); pos;)
...
void CCFIMaskedEdit::SetMask(LPCTSTR pszMask)
{
	if(pszMask==NULL)
	{
		pszMask=_T("");
...
CString CCFIMaskedEdit::GetInputData() const
{
	CString csInputData;
...
CString CCFIMaskedEdit::GetInputData(LPCTSTR lpszText) const
{
...
BOOL CCFIMaskedEdit::SetInputData(LPCTSTR pszInputData, int nBeginPos/*=0*/, 
...
TCHAR CCFIMaskedEdit::GetPromptSymbol()
{
	return m_chPromptSymbol;
}

void CCFIMaskedEdit::SetPromptSymbol(TCHAR chNewPromptSymbol)
{
...
void CCFIMaskedEdit::EmptyData(BOOL bOnlyInput/*=FALSE*/)
{
	if(m_listData.GetCount()==0)
	{
		DeleteContents();
...
BOOL CCFIMaskedEdit::IsInputEmpty()
{
	if(m_listData.GetCount()==0)
	{
		CString csInputData;
...
BOOL CCFIMaskedEdit::GetInsertMode() const
{
...
void CCFIMaskedEdit::SetInsertMode(BOOL bInsertMode)
{
	// The standard CEdit control does not support over-typing. 
	// This flag is used to manage over-typing internally. 
...
BOOL CCFIMaskedEdit::GetAutoTab() const
{
...
void CCFIMaskedEdit::SetAutoTab(BOOL bAutoTab)
{
	// The standard CEdit control does not support AutoTab mode. 
...
CString CCFIMaskedEdit::ShowMask() const
{
	CString csShow;
	CMaskData* pobjData=NULL;
	for(POSITION pos=m_listData.GetHeadPosition(); pos;)
...
BOOL CCFIMaskedEdit::IsInputData(int nPosition) const
{
	if(m_listData.GetCount()==0)
	{
		return TRUE;
...
int CCFIMaskedEdit::DeleteRange(int nSelectionStart, int nSelectionEnd)
{
...
int CCFIMaskedEdit::InsertAt(int nSelectionStart, TCHAR chNewChar)
{
...
int CCFIMaskedEdit::SetAt(int nSelectionStart, TCHAR chNewChar)
{
...
int CCFIMaskedEdit::GetNextInputLocation(int nSelectionStart)
{
...
int CCFIMaskedEdit::GetPreviousInputLocation(int nSelectionStart)
{
	// One of the functions of this edit control is that it skips over literals. 
	// We need a function to help skip to the next position. 
	int nNextInputLocation=nSelectionStart;
...
int CCFIMaskedEdit::GetEmptyInputLocation(int nSelectionStart)
{
...
void CCFIMaskedEdit::Update(int nSelectionStart/*=0*/)
{
...
void CCFIMaskedEdit::UpdateInsertionPointForward(int nSelectionStart)
{
...
void CCFIMaskedEdit::UpdateInsertionPointBackward(int nSelectionStart)
{
	int nNewInsertionPoint=GetPreviousInputLocation(nSelectionStart);
	SetSel(nNewInsertionPoint, nNewInsertionPoint);
}

void CCFIMaskedEdit::ValidationError()
{
	::MessageBeep(MB_ICONEXCLAMATION);
}

/////////////////////////////////////////////////////////////////////////////
// CCFIMaskedEdit overrides

void CCFIMaskedEdit::PreSubclassWindow() 
{
...
/////////////////////////////////////////////////////////////////////////////
// CCFIMaskedEdit message handlers

BOOL CCFIMaskedEdit::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CCFIMaskedEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
void CCFIMaskedEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	// If there is no mask, then exit quickly performing the default operation. 
	if(m_listData.GetCount()==0 || GetStyle()&ES_READONLY)
	{
...
void CCFIMaskedEdit::OnSetFocus(CWnd* pOldWnd) 
{
...
LONG CCFIMaskedEdit::OnCut(UINT wParam, LONG lParam)
{
...
LONG CCFIMaskedEdit::OnCopy(UINT wParam, LONG lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
LONG CCFIMaskedEdit::OnPaste(UINT wParam, LONG lParam) 
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
LONG CCFIMaskedEdit::OnClear(UINT wParam, LONG lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
void CCFIMaskedEdit::OnKillfocus() 
{
...
BOOL CCFIMaskedEdit::OnValidate() 
{
	// by default return TRUE
	// one can overwrite this function to provide validation capability
	// in CCFIMaskedEdit derived class
	return TRUE;
}

void CCFIMaskedEdit::OnLButtonDown(UINT nFlags, CPoint point)
{
...
LONG CCFIMaskedEdit::OnSetText(UINT wParam, LONG lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
...
int CCFIMaskedEdit::RPtoLP(int nRealPos) const 
{
	// All CCFIMaskedEdit functions that take a cursor position as argument interpret it
...
int CCFIMaskedEdit::LPtoRP(int nLogicalPos) const 
{
	// All CCFIMaskedEdit functions that take a cursor position as an argument interpret it
	// as a real position within the edit control (taking into account all symbols including 
...
BOOL CCFIMaskedEdit::NotifyParent(UINT nNotificationID)
{
	CWnd* pParentWnd=GetParent();
	if(pParentWnd==NULL)
		return FALSE;
