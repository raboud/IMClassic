...
class OX_CLASS_DECL COXRegistry : public CObject
{
...
public:
	...
	// --- Out : 
	// --- Returns : succeeded or not
	// --- Effect : The DeleteKey function deletes a key and all its
	// 				descendents.

	virtual BOOL DeleteAllValuesOfKey( LPCTSTR pszNameOfKeyToDeleteValues, BOOL bRecursive = TRUE );
	// --- In  : pszNameOfKeyToDelete :	Points to a null-terminated string specifying
	// 				the name of the key which values to delete. This parameter cannot be NULL
	//			 bRecursive : if TRUE deletes all values of subkeys, otherwise deletes only values
	//						  of given key.
	// --- Out : 
	// --- Returns : succeeded or not
	// --- Effect : Deletes all the values of a key
	...
	virtual BOOL EnumerateKeys(DWORD dwSubkeyIndex, CString& sSubkeyName,
		CString& sClassName );
	...
	virtual BOOL EnumerateValues(DWORD dwValueIndex, CString& sNameOfValue,
		KeyValueTypes& eTypeCode, LPBYTE pbDataBuffer, DWORD& dwSizeOfDataBuffer);
	...
	virtual BOOL GetSecurity(const SECURITY_INFORMATION& whatYouWantToKnow,
		PSECURITY_DESCRIPTOR pDataBuffer, DWORD& dwSizeOfDataBuffer);
	...
	virtual BOOL NotifyChange(HANDLE hEventHandle=NULL,
		NotifyChangeFilter eChangesToBeReported   = notifyLastSet,
		BOOL bThisSubkeyOrAllSubkeys              = changeSpecifiedKeyOnly,
		BOOL bWaitForChangeOrSignalEvent          = WAIT_FOR_CHANGE );
	...
	virtual BOOL OpenKey(LPCTSTR pszNameOfSubkeyToOpen,
		CreatePermissions eSecurityAccessMask = permissionAllAccess);
	...
	virtual BOOL ReplaceFile( LPCTSTR pszNameOfSubkey, LPCTSTR pszNameOfFileWithNewData,
	...
	virtual BOOL RestoreFromFile( LPCTSTR pszNameOfFileHoldingSavedTree, 
		DWORD dwVolatilityFlags = NULL );
	...
	virtual BOOL SaveToFile( LPCTSTR pszNameOfFileToHoldTree, 
		LPSECURITY_ATTRIBUTES pSecurityAttributes = NULL );
	...
	virtual BOOL SetStringArrayValue( LPCTSTR pszNameOfValue, 
		const CStringArray& asStringArray );
	// --- In  : pszNameOfValue : Points to a null-terminated string containing the name
	// 				of the value to be set. 
	...
	virtual BOOL SetValue(LPCTSTR pszNameOfSubkey, KeyValueTypes eTypeOfValueToSet,
		CONST PBYTE pbAddressOfValueData, DWORD dwSizeOfData );
...
private:
	...
	LONG DeleteAllValuesOfKey( HKEY hKeyToDeleteValues, BOOL bRecursive);
