...
void COXRegistry::m_Initialize()
	...
	*/
	...
	if(m_KeyHandle!=NULL && m_bHaveToCloseKeyHandle)
	{
...
LONG COXRegistry::DeleteRegistryKey(HKEY hParentKey, LPCTSTR pszNameOfKeyToDelete)
	...
	lStatus2 = ::RegOpenKeyEx(hParentKey, pszNameOfKeyToDelete, NULL, 
		permissionAllAccess, &hKeyToDelete);
...
BOOL COXRegistry::DeleteAllValuesOfKey( LPCTSTR pszNameOfKeyToDeleteValues, BOOL bRecursive /* = TRUE */)
	{
	ASSERT_VALID( this );
	ASSERT( pszNameOfKeyToDeleteValues != NULL );

	if ( pszNameOfKeyToDeleteValues == NULL )
		{
		TRACE0("In COXRegistry::DeleteAllValuesOfKey : Invalid parameter\n");
		m_ErrorCode = ERROR_INVALID_PARAMETER;
		return( FALSE );
		}

	HKEY hKeyDeleteValues;
	m_ErrorCode = ::RegOpenKeyEx( m_RegistryHandle, pszNameOfKeyToDeleteValues, 
		NULL, permissionAllAccess, &hKeyDeleteValues );

	if (m_ErrorCode != ERROR_SUCCESS)
		{
		TRACE1("In COXRegistry::DeleteAllValuesOfKey : Subkey opening Failed code = %lu\n", m_ErrorCode);
		return(FALSE);
		}
	
	m_ErrorCode = DeleteAllValuesOfKey(hKeyDeleteValues, bRecursive);
	::RegCloseKey(hKeyDeleteValues);
...
BOOL COXRegistry::EnumerateKeys(DWORD dwSubkeyIndex, CString& sSubkeyName, 
								CString& sClassName)
	{
	ASSERT_VALID( this );
	...
	m_ErrorCode = ::RegEnumKeyEx( m_KeyHandle, 
		dwSubkeyIndex, subkey_name_string, &size_of_subkey_name_string,
		NULL, class_name_string, &size_of_class_name_string, &m_LastWriteTime );
...
BOOL COXRegistry::EnumerateValues(DWORD    dwValueIndex, 
	...
	*/
	...
	m_ErrorCode = ::RegEnumValue( m_KeyHandle, dwValueIndex,
		temp_name, &temp_name_size, NULL, &temp_type_code,
		pbDataBuffer, &dwSizeOfDataBuffer );
...
/*
	// must handle as memomory exception
...
BOOL COXRegistry::GetSecurity( const SECURITY_INFORMATION& whatYouWantToKnow,
	...
	m_ErrorCode = ::RegGetKeySecurity( m_KeyHandle, whatYouWantToKnow,
		pDataBuffer, &dwSizeOfDataBuffer );
...
BOOL COXRegistry::NotifyChange( HANDLE           hEventHandle, 
                              NotifyChangeFilter eChangesToBeReported,
                              BOOL               bThisSubkeyOrAllSubkeys,
                              BOOL               bWaitForChangeOrSignalEvent )
	{
	ASSERT_VALID( this );

	m_ErrorCode = ::RegNotifyChangeKeyValue( m_KeyHandle, bThisSubkeyOrAllSubkeys,
		eChangesToBeReported, hEventHandle, bWaitForChangeOrSignalEvent );
...
BOOL COXRegistry::OpenKey(LPCTSTR pszNameOfSubkeyToOpen, 
						  CreatePermissions eSecurityAccessMask)
	{
	ASSERT_VALID( this );
	...
	*/
	...
	m_ErrorCode = ::RegOpenKeyEx( m_RegistryHandle, pszNameOfSubkeyToOpen, NULL, 
		eSecurityAccessMask, &m_KeyHandle );
...
BOOL COXRegistry::QueryInfo()
	...
	m_ErrorCode = ::RegQueryInfoKey( m_KeyHandle, class_name,
		&size_of_class_name, (LPDWORD) NULL, &m_NumberOfSubkeys,
		&m_LongestSubkeyNameLength, &m_LongestClassNameLength,
		&m_NumberOfValues, &m_LongestValueNameLength,
		&m_LongestValueDataLength, &m_SecurityDescriptorLength,
		&m_LastWriteTime );
...
BOOL COXRegistry::QueryValue( LPCTSTR        pszNameOfValue, 
	...
	*/
	...
	m_ErrorCode = ::RegQueryValueEx( m_KeyHandle, (LPTSTR) pszNameOfValue, 
		NULL, &temp_data_type, pbAddressOfBuffer, &dwSizeOfBuffer );
...
BOOL COXRegistry::ReplaceFile( LPCTSTR pszNameOfSubkey,
	...
	m_ErrorCode = ::RegReplaceKey( m_KeyHandle, pszNameOfSubkey,
		pszNameOfFileWithNewData, pszNameOfBackupFile );
...
BOOL COXRegistry::RestoreFromFile(LPCTSTR pszNameOfFileHoldingSavedTree, 
								  DWORD dwVolatilityFlags )
	{
	ASSERT_VALID( this );
	...
	m_ErrorCode = ::RegRestoreKey( m_KeyHandle, pszNameOfFileHoldingSavedTree,
		dwVolatilityFlags );
...
BOOL COXRegistry::SaveToFile( LPCTSTR pszNameOfFileToHoldTree, LPSECURITY_ATTRIBUTES pSecurityAttributes )
	...
	m_ErrorCode = ::RegSaveKey( m_KeyHandle, pszNameOfFileToHoldTree, 
		pSecurityAttributes );
...
BOOL COXRegistry::SetSecurity( const SECURITY_INFORMATION& securityInformation,
	...
	m_ErrorCode = ::RegSetKeySecurity( m_KeyHandle, securityInformation, 
		pSecurityDescriptor );
...
BOOL COXRegistry::SetValue( LPCTSTR             pszNameOfSubkey, 
	...
	m_ErrorCode = ::RegSetValueEx( m_KeyHandle, pszNameOfSubkey, 0,
		eTypeOfValueToSet, pbAddressOfValueData, dwSizeOfData );
...
BOOL COXRegistry::CopyRegistryKey(LPCTSTR pszToKey, BOOL bReplace /* = FALSE */)
	...
	//  to "walk the tree"
	BOOL bResult=CopyRegistryKey(m_KeyHandle, hToKeyHandle, bReplace);
	::RegCloseKey(hToKeyHandle);
	...
	do
		...
		if( lStatus == ERROR_SUCCESS)						//  Was a valid key found?
			...
			if(lStatus2 == ERROR_SUCCESS)					//  If the key opened...
				...
				lStatus2 = ::RegOpenKeyEx(hFromKey, sValueName, NULL, 
					permissionAllAccess, &hNewFrom);
				if(lStatus2 == ERROR_SUCCESS)
...
LONG COXRegistry::DeleteAllValuesOfKey( HKEY hKeyToDeleteValues, BOOL bRecursive)
	{
	//  Start with the highest index of Values
	DWORD dwValue, dwKey;
	long lStatus, lStatus2=NULL;
	DWORD dwValueSize;
	CString sValueName;
	HKEY hSubKeyToDeleteValues;

	lStatus = ::RegQueryInfoKey(hKeyToDeleteValues, NULL, NULL, NULL, &dwKey, NULL, NULL, &dwValue, NULL, NULL, NULL, NULL);
	if (lStatus != ERROR_SUCCESS)
		return lStatus;
	...
	do 
		...
		//  and enumerate data values at current key
		lStatus = ::RegEnumValue(hKeyToDeleteValues, --dwValue, sValueName.GetBuffer(dwValueSize), &dwValueSize,
			NULL, NULL, NULL, NULL);
		sValueName.ReleaseBuffer();
		
		if(lStatus == ERROR_SUCCESS)           //  Delete each value
			lStatus2 = ::RegDeleteValue(hKeyToDeleteValues, (LPCTSTR)sValueName);
		} 
	while(lStatus == ERROR_SUCCESS && lStatus2 == ERROR_SUCCESS);			//  Loop until all values found
	...
	if (bRecursive)
		...
		do
			...
			//  Get "next" key
			lStatus = ::RegEnumKeyEx(hKeyToDeleteValues, --dwKey, sValueName.GetBuffer(dwValueSize), &dwValueSize,
				NULL, NULL, NULL, &FileTime);
			sValueName.ReleaseBuffer();
			
			if( lStatus == ERROR_SUCCESS)						//  Was a valid key found?
				{                                               //  Open the key if found
				lStatus2 = ::RegOpenKeyEx(hKeyToDeleteValues, sValueName, NULL, 
					permissionAllAccess, &hSubKeyToDeleteValues);
				if(lStatus2 == ERROR_SUCCESS)
					{													//  If that worked, recurse back here
					DeleteAllValuesOfKey(hSubKeyToDeleteValues, bRecursive);		//  walk the tree and delete values
					::RegCloseKey(hSubKeyToDeleteValues);							//  Close each subkey again
					}
