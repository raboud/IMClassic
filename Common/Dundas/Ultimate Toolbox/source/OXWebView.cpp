...
BEGIN_MESSAGE_MAP(COXWebView, CView)
	...
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_COMMAND(ID_WEBVIEW_BACK, OnWebviewBack)
	ON_COMMAND(ID_WEBVIEW_FORWARD, OnWebviewForward)
	ON_UPDATE_COMMAND_UI(ID_WEBVIEW_BACK, OnUpdateWebviewBack)
	ON_UPDATE_COMMAND_UI(ID_WEBVIEW_FORWARD, OnUpdateWebviewForward)
	ON_COMMAND(ID_WEBVIEW_HOME, OnWebviewHome)
	ON_COMMAND(ID_WEBVIEW_SEARCH, OnWebviewSearch)
	ON_UPDATE_COMMAND_UI(ID_WEBVIEW_STOP, OnUpdateWebviewStop)
	ON_COMMAND(ID_WEBVIEW_STOP, OnWebviewStop)
	ON_UPDATE_COMMAND_UI(ID_FILE_PRINT_PREVIEW, OnUpdateFilePrintPreview)
	ON_COMMAND(ID_FILE_PRINT, OnFilePrint)
	ON_UPDATE_COMMAND_UI(ID_WEBVIEW_REFRESH, OnUpdateWebviewRefresh)
	ON_COMMAND(ID_WEBVIEW_REFRESH, OnWebviewRefresh)
	//}}AFX_MSG_MAP
...
int COXWebView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
	...
	HINSTANCE hInst = AfxFindResourceHandle(MAKEINTRESOURCE(IDR_WEBVIEW_BAR), RT_TOOLBAR);
	HRSRC hRsrc = ::FindResource(hInst, MAKEINTRESOURCE(IDR_WEBVIEW_BAR), RT_TOOLBAR);
	ASSERT(hRsrc != NULL);
#endif

	// Fail if we cannot create the toolbar.
	if (!m_Toolbar.Create(this) ||
		!m_Toolbar.LoadToolBar(IDR_WEBVIEW_BAR))
		{
