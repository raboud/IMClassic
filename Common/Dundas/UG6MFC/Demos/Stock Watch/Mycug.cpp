...
void MyCug::OnGetCell(int col,long row,CUGCell *cell)
{
	...
	if(row >= 0)
	{
		...
		else if(col == 6)
		{
			...
			if(value >= 1000000)
			{
				sprintf(szBuffer, "%d,%03d,%03d", (int)value/1000000, (int)(value - (int)value/1000000*1000000)/1000, (int)(value - (int)value/1000000*1000000)%1000);
			}
			else
			{
				sprintf(szBuffer, "%d,%03d", (int)value/1000, (int)value%1000);
			}
...
void MyCug::OnTimer(UINT nIDEvent)
{
	...
	else if(nIDEvent == 1)	// Data changes
	{
		if((rand()%5) >= 3)
		{
			...
			fBid += 12. - rand() / (RAND_MAX/20.);
			fAsk = fBid + rand() / (RAND_MAX/2.);
			fLast = fBid + rand() / (RAND_MAX/2.);
			fNet = 3.5 - rand() / (RAND_MAX/6.);
			fVolume += (int)((rand() / (RAND_MAX/1000.)))*100;

			cellLast.SetParam((int)rand() % 2);

			// Set new data into the cell
			sprintf(szBuffer, "%.02f", fBid);
			cellBid.SetText(szBuffer);
			sprintf(szBuffer, "%.02f", fAsk);
			cellAsk.SetText(szBuffer);
			sprintf(szBuffer, "%.02f", fLast);
			cellLast.SetText(szBuffer);
			sprintf(szBuffer, "%+#.02f", fNet);
			cellNet.SetText(szBuffer);
			sprintf(szBuffer, "%d", (int)fVolume);
