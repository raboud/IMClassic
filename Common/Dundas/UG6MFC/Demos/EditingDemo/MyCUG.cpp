...
int MyCug::OnEditStart(int col, long row,CWnd **edit)
{
	...
	{
		if ( ! m_coxMaskEdit.GetSafeHwnd())
		{
			m_coxMaskEdit.Create( WS_CHILD|WS_VISIBLE, CRect(0,0,0,0), this, COXMASK_ID );
			m_coxMaskEdit.m_ctrl = this;
		}
	...
	{
		if ( ! m_coxNumeric.GetSafeHwnd())
		{
			m_coxNumeric.Create( WS_CHILD|WS_VISIBLE, CRect(0,0,0,0), this, COXMASK_ID );
			m_coxNumeric.m_ctrl = this;
		}
		m_coxNumeric.SetDecimalSeparator( 'a' );
		m_coxNumeric.SetGroupSeparator( ' ' );
		...
		if ( CString(cell.GetText()) != "<Disabled>" )
			m_coxNumeric.SetDecimalDigitCount((int)cell.GetNumber());
		else
			m_coxNumeric.SetDecimalDigitCount( -1 );
		GetCell( col, 37, &cell );	// Decimal Separator
		m_coxNumeric.SetDecimalSeparator( CString(cell.GetText()).GetAt(0));
		GetCell( col, 38, &cell );	// Fractional digit count
		m_coxNumeric.SetFractionalDigitCount((int)cell.GetNumber());
		GetCell( col, 39, &cell );	// Group Separator
		m_coxNumeric.SetGroupSeparator( CString(cell.GetText()).GetAt(0));
		GetCell( col, 40, &cell );	// Digits in group
		if ( CString(cell.GetText()) != "<Disabled>" )
			m_coxNumeric.SetGroupLength((int)cell.GetNumber());
		else
			m_coxNumeric.SetDecimalDigitCount( -1 );
		GetCell( col, 41, &cell );	// Negative Format
		m_coxNumeric.SetNegativeFormat( cell.GetText());
		GetCell( col, 42, &cell );	// Show leading zero
		m_coxNumeric.SetShowLeadingZero( cell.GetBool());
	...
	{
		if ( ! m_coxCurrency.GetSafeHwnd())
		{
			m_coxCurrency.Create( WS_CHILD|WS_VISIBLE, CRect(0,0,0,0), this, COXMASK_ID );
			m_coxCurrency.m_ctrl = this;
		}
		m_coxCurrency.SetDecimalSeparator( 'a' );
		m_coxCurrency.SetGroupSeparator( ' ' );

		// setup properties of the currency edit control (located in rows 36 to 44)
		GetCell( col, 36, &cell );	// Decimal digit count
		if ( CString(cell.GetText()) != "<Disabled>" )
			m_coxCurrency.SetDecimalDigitCount((int)cell.GetNumber());
		else
			m_coxCurrency.SetDecimalDigitCount( -1 );
		GetCell( col, 37, &cell );	// Decimal Separator
		m_coxCurrency.SetDecimalSeparator( CString(cell.GetText()).GetAt(0));
		GetCell( col, 38, &cell );	// Fractional digit count
		m_coxCurrency.SetFractionalDigitCount((int)cell.GetNumber());
		GetCell( col, 39, &cell );	// Group Separator
		m_coxCurrency.SetGroupSeparator( CString(cell.GetText()).GetAt(0));
		GetCell( col, 40, &cell );	// Digits in group
		if ( CString(cell.GetText()) != "<Disabled>" )
			m_coxCurrency.SetGroupLength((int)cell.GetNumber());
		else
			m_coxCurrency.SetDecimalDigitCount( -1 );
		GetCell( col, 41, &cell );	// Positive Format
		m_coxCurrency.SetPositiveFormat( cell.GetText());
		GetCell( col, 42, &cell );	// Negative Format
		m_coxCurrency.SetNegativeFormat( cell.GetText());
		GetCell( col, 43, &cell );	// Currency name
		m_coxCurrency.SetCurrencyName( cell.GetText());
		GetCell( col, 44, &cell );	// Show leading zero
		m_coxCurrency.SetShowLeadingZero( cell.GetBool());
