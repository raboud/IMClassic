...
void CUGCtrl::Moved()
{
	...
	//let the current cell know if the screen has scrolled
	if(scrolled){
		m_cell.ClearAll();
		GetCellIndirect(m_GI->m_currentCol,m_GI->m_currentRow,&m_cell);
		GetCellType(m_cell.GetCellType())->
			OnScrolled(m_GI->m_currentCol,m_GI->m_currentRow,&m_cell);
	}
...
int CUGCtrl::MoveTopRow(int flag){
	...
	switch(flag){
		...
		case UG_LINEUP:{
            ...
            {
                i = -1;
                while((GetRowHeight(m_GI->m_topRow + i) == 0) && (m_GI->m_topRow + i >= 0))
                    --i;
            }
		...
		case UG_LINEDOWN:{
            ...
            {
                i = 1;
                while((GetRowHeight(m_GI->m_topRow + i) == 0) && (m_GI->m_topRow + i < m_GI->m_numberRows))
                    ++i;
            }
			return SetTopRow(m_GI->m_topRow +i);
		}
...
/***************************************************
	...
	Return
****************************************************/
int CUGCtrl::AddMenuItem(int ID,LPCTSTR string, bool bGrayed){
	...
	{
		int nFlags = (bGrayed) ? MF_GRAYED : 0;
		if(m_menu->AppendMenu(MF_STRING | nFlags,ID,string) != FALSE)
			return UG_SUCCESS;
	}
...
****************************************************/
int CUGCtrl::OnHint(int col,long row,int section,CString *string, int *maxwidth /*=NULL*/){
	UNREFERENCED_PARAMETER(section);
