...
BOOL CUGGrid::ToolTipNeedText( UINT id, NMHDR* pTTTStruct, LRESULT* pResult )
{
	...
	{
		if ( m_ctrl->OnHint( col, row, UG_GRID, &string, &maxwidth ) == TRUE )
		{
			TOOLTIPTEXT *pTTT = (TOOLTIPTEXT *)pTTTStruct;
			// Do this to Enable multiline ToolTips
			::SendMessage( pTTT->hdr.hwndFrom, TTM_SETMAXTIPWIDTH, 0, maxwidth );
...
BOOL CUGGrid::OnHelpInfo(HELPINFO* pHelpInfo) 
{
	if (pHelpInfo->iContextType == HELPINFO_WINDOW)
	{
		...
		AfxGetApp()->WinHelp( m_ctrl->OnGetContextHelpID( col, row, UG_GRID ));
