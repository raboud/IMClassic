...
int CUGODBCDatasource::Browse(HWND hwnd){
	...
	// for table info....
	char	szTableQualifier[128];
	char	szTableOwner[128];
	...
	if (retcode == SQL_ERROR) {
		AfxMessageBox("Probable memory allocation error in SQLAllocEnv");
		return (int) retcode;
	}
	...
	if (retcode == SQL_ERROR) {
		ReportSQLError(henv, SQL_NULL_HDBC, SQL_NULL_HSTMT, retcode, "SQLAllocConnect");
		SQLFreeEnv(henv);
		return (int) retcode;
	}		
	...
	if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
		...
		retcode = SQLDriverConnect(hdbc,hwnd,(UCHAR FAR *)"",
							0,buf,254,&num,SQL_DRIVER_PROMPT);

							
		if (retcode != SQL_SUCCESS) {
			#ifdef UG_REPORT_ERROR
				ReportSQLError(henv, hdbc, SQL_NULL_HSTMT, retcode, "SQLDriverConnect");
			#endif
		...
		if(retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO){
			...
			switch (retcode) {
				...
				case SQL_SUCCESS_WITH_INFO:{
					AfxMessageBox("SQLAllocStmt returned success with info");
					...
					AfxMessageBox((LPCSTR)sqlstate);
					AfxMessageBox((LPCSTR)sqlerror);
					break;
				}
				case SQL_NO_DATA_FOUND:{
					AfxMessageBox("SQLAllocStmt returned no data found");
					break;
				}
				case SQL_ERROR:{
					AfxMessageBox("SQLAllocStmt returned sql error");
				...
				case SQL_INVALID_HANDLE:{
					AfxMessageBox("SQLDriverConnect returned invalid handle");
					break;
				}
				default:
					break;
                ...
                if (retcode == SQL_ERROR || retcode == SQL_SUCCESS_WITH_INFO) {
					ReportSQLError(henv, hdbc, hstmt, retcode, "SQLTables");
				...
				if (retcode == SQL_ERROR || retcode == SQL_SUCCESS_WITH_INFO)
					ReportSQLError(henv, hdbc, hstmt, retcode, "SQLFetch");

				if (retcode != SQL_NO_DATA_FOUND)
					m_connectString = (LPCSTR)buf2;	// save connection string!


                // Subsequent calls to GetTableInfo will be able to 
                // retrieve this information...
				while (retcode != SQL_NO_DATA_FOUND) {
					strcpy(m_dsInfoArray[m_tableCount].tableQualifier,szTableQualifier);
					strcpy(m_dsInfoArray[m_tableCount].tableOwner, szTableOwner);
					strcpy(m_dsInfoArray[m_tableCount].tableName,szTableName);
					strcpy(m_dsInfoArray[m_tableCount].tableType,szTableType);
					strcpy(m_dsInfoArray[m_tableCount].remarks,szRemarks);
...
****************************************************/
int CUGODBCDatasource::Open(LPCSTR database,LPCSTR table){
	return Open(database,table,0,CRecordset::dynaset,0,NULL,NULL);
}
/***************************************************
****************************************************/
int CUGODBCDatasource::OpenSQL(LPCSTR database,LPCSTR sql){
	return OpenSQL(database,sql,0,CRecordset::dynaset,0,NULL,NULL);
}
/***************************************************
****************************************************/
int CUGODBCDatasource::Open(LPCSTR database,LPCSTR table,DWORD dbOptions,UINT recType,DWORD recOptions,LPCSTR user,LPCSTR pass){
	return OpenSQL(database,table,dbOptions,recType,recOptions,user,pass);
}
/***************************************************
****************************************************/
int CUGODBCDatasource::OpenSQL(LPCSTR database,LPCSTR sql,DWORD dbOptions,UINT recType,DWORD recOptions,LPCSTR user,LPCSTR pass){
	...
	if(!m_connectString.IsEmpty()) {
		wsprintf(buf,"%s",m_connectString);		// from OpenEx()
	} else {

		//create the connect string
		if(user!=NULL && pass!=NULL){
			wsprintf(buf,"ODBC;DSN=%s;UID=%s;PWD=%s",database,user,pass);
		}
		else if(user!=NULL){
			wsprintf(buf,"ODBC;DSN=%s;UID=%s",database,user);
		}
		else if(pass!=NULL){
			wsprintf(buf,"ODBC;DSN=%s;PWD=%s",database,pass);
		}
		else{
			wsprintf(buf,"ODBC;DSN=%s",database);
		}
	}
...
//        cell->SetAlignment(UG_ALIGNRIGHT);
	...
	if(m_Record->IsFieldNull(cfi->pValue)) {
		...
		switch(cfi->nSQLType){
			...
			case SQL_TINYINT:{
				string.Format("%d",*(unsigned  char *) cfi->pValue);
				break;	
			}
			case SQL_TIMESTAMP:{
			...
			case SQL_DATE:{		
				string = ((CTime*)(cfi->pValue))->Format( "%d %b %Y" );
				break;
			}
			case SQL_TIME:{
				string = ((CTime*)(cfi->pValue))->Format( "%H:%M:%S" );
				break;
			}
			...
			case SQL_SMALLINT:{
				string.Format("%d",*(int *) cfi->pValue);
				break;
			}
			case SQL_INTEGER:{
				string.Format("%ld",*(long *) cfi->pValue);
				break;
			}
			case SQL_NUMERIC:
			case SQL_DECIMAL:
			case SQL_REAL:
			case SQL_FLOAT:{
				string.Format("%.2f",*(float *) cfi->pValue);
				break;
			}
			case SQL_DOUBLE:{
				string.Format("%.2f",*(double *) cfi->pValue);
				break;
			}
			case SQL_BIGINT:
				string.Format("%.0f",*(double *) cfi->pValue);
				break;
			default: {
				AfxMessageBox("Hey - type not found!");
					 }
		}
	}	
...
int CUGODBCDatasource::AppendRow(){
	...
	try{
		...
		if(!dirty)
			TRACE("Warning: no dirty fields on update in AppendRow()\n");
...
{
	CHAR 			buf[512];
	UCHAR FAR 		szSqlState[255];	
	SDWORD			pfNativeError;	
	UCHAR FAR		szErrorMsg[254];	
...
int CUGODBCDatasource::SetFieldValueFromCell(int col, CUGCell *cell) {
	...
	switch(cfi->nSQLType){
		...
		case SQL_BIGINT:
		case SQL_DOUBLE:{
			*(double*) cfi->pValue = atof(text);
			break;
		}
		case SQL_INTEGER:{
			*(long*) cfi->pValue  = atol(text);
			break;
		}
		case SQL_BIT:{
			if(atol(text))
		...
		case SQL_DECIMAL:
		case SQL_FLOAT:{
			*(float*) cfi->pValue = (float)atof(text);
			break;
		}
		case SQL_SMALLINT:{
			*(int*) cfi->pValue = atoi(text);
			break;
		}
		case SQL_TINYINT:{
			*(unsigned char*) cfi->pValue = text[0];
			break;	
		}
		case SQL_BINARY:
		case SQL_VARBINARY:
...
int CUGODBCDatasource::SortBy(int *cols,int num,int flags)
{
    ...
    try
    {
		...
		for(loop = 0;loop < num;loop++) 
		{
			if ( loop > 0 )
			{
				strColName.Format( ", %d", cols[loop] + 1 );
			}
			else 
			{
				strColName.Format( "%d", cols[loop] + 1 );
