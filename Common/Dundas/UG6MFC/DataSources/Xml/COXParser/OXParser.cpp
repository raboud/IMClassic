...
{ 
    ...
    {
        _stprintf(strValue, TEXT("%d"), m_nValue);
        return strValue;
    }
...
LPTSTR COXParser::GetCharEntity(TCHAR ch)
{
    ...
    while (pos)
    {
        ...
        {
            _stprintf(szEntity, TEXT("&%s;"), szName);
            return szEntity;
        }
    }
...
{   
    ...
    sprintf(buffer, "<%s", T2A((LPTSTR)pElement->GetName()));
    if (!::WriteFile(hFile, buffer, strlen(buffer), &nCount, NULL))
        return FALSE;
