...
//		DBTYPE_VARIANT:
	...
	case DBTYPE_UI1:
		temp.Format("%d", *(BYTE*)(m_buffer));
		break;
	case DBTYPE_I1:
		temp.Format("%c", *(signed char*)(m_buffer));
		break;
	case DBTYPE_I2:
		temp.Format("%d", *(int*)(m_buffer));
		break;
	case DBTYPE_UI2:
		temp.Format("%u", *(unsigned int*)(m_buffer));
		break;
	case DBTYPE_I4:
		temp.Format("%d", *(long*)m_buffer);
		break;
	case DBTYPE_UI4:
		temp.Format("%u", *(unsigned long*)m_buffer);
		break;
	case DBTYPE_R4:
		temp.Format("%f", *(float*)m_buffer);
		break;
	case DBTYPE_R8:
		temp.Format("%f", *(float*)m_buffer);
		break;
...
//		temp.Format("%u", *(unsigned long*)m_buffer);
	...
	case DBTYPE_STR:
		...
		{
			strcpy ((LPSTR)m_buffer, str);
			m_Length=str.GetLength();
		}
		else
		{
			TRACE ("Error: string is too long");
			return FALSE;
		}
		break;
	case DBTYPE_WSTR:
		if(str.GetLength()<m_MaxLength)
		{
			mbstowcs((LPWSTR)m_buffer, str, str.GetLength());
			m_Length=str.GetLength();
		}
		else
		{
