...
int	CUGDAODataSource::GetCell(int col,long row,CUGCell *cell){
	...
	if(col <0){
		...
		if(m_ctrl->GetCurrentRow() == row){
			...
			if(row == m_nGhostRow) {
				...
				if(m_bNumberSideHdg) {
					_stprintf(m_buf, _T("%ld"), row+m_nSH_NumBase);
	...
	switch(m_coVariant.vt){		
		...
		case VT_BSTR:			
            ...
            break;
		case VT_UI1:
			wsprintf(m_buf,_T("%d"),m_coVariant.bVal);
			cell->SetText(m_buf);
            break;
		case VT_I2:
			wsprintf(m_buf,_T("%d"),(int)m_coVariant.iVal);
			cell->SetText(m_buf);
            break;
		case VT_I4:
			wsprintf(m_buf,_T("%ld"),m_coVariant.lVal);
			cell->SetText(m_buf);
            break;
		case VT_R4:
			_stprintf(m_buf,_T("%f"),m_coVariant.fltVal);
			cell->SetText(m_buf);
            break;
		case VT_R8:
			_stprintf(m_buf,_T("%lf"),(double)m_coVariant.dblVal);
			cell->SetText(m_buf);
            break;
		case VT_BOOL:
			if(m_coVariant.iVal)
	...
	for(int i = 0; i < cols; i++) {
		...
		if((m_FieldInfo[i].m_bRequired) || (!m_FieldInfo[i].m_bAllowZeroLength)){
			...
			switch(m_FieldInfo[i].m_nType){
				...
				case dbCurrency:
					...
					try{
						...
						CString cs;
						GetColName(i,&cs);
						_tcscpy(buf,_T("The field '"));
						_tcscat(buf,(LPCTSTR) cs);
						_tcscat(buf, _T("' failed conversion - "));
			...
			try{
				...
				_tcscpy(buf,_T("The field '"));
				_tcscat(buf,(LPCTSTR) cs);
				_tcscat(buf, _T("' failed validation - "));
				_tcscat(buf, e->m_pErrorInfo->m_strDescription);
				AfxMessageBox(buf);
				//e->ReportError();
				e->Delete();
                Reset();
