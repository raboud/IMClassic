...
CString CAutoRichEditCtrl::GetPlainText()
{
	EDITSTREAM es;
	es.dwError = 0;
	es.pfnCallback = CBStreamOut;		// Set the callback
	...
	//StreamOut(SF_RTFNOOBJS, es);   
	///

	return sText;
}

int CAutoRichEditCtrl::GetParagraphContents(CStringArray& strParagraphs, bool bPlainText)
{
	// this function returns an array of strings containing the paragraphs in the Rich Edit control.
	// useful for combining one rich edit control's contents with an existing one

	int iNumParagraphs = 0;

	CString strRTFPlain = GetPlainText();
	if (strRTFPlain.GetLength() == 0)
	{
		return 0;
	}
	...
	if (bPlainText)
	{
		CString strDoc = GetPlainText();
		int iPos = 0;

		CString strSub = strDoc.Tokenize("\r\n", iPos);
		while (strSub != "")
		{
			strParagraphs.Add(strSub);
			strSub = strDoc.Tokenize("\r\n", iPos);
		}
	}
	else
	{
		CString strRTFDoc = GetRTF();
		CString strSub = "";

		if (strRTFDoc.GetLength() > 0)
		{
			int iStart = strRTFDoc.Find("\\par", 0);
			while (iStart != -1)
			{
				int iStop = strRTFDoc.Find("\\par", iStart+4);
				if (iStop > iStart)
				{
					strSub = strRTFDoc.Mid(iStart, iStop-iStart+4);
					strParagraphs.Add(strSub);
					iNumParagraphs++;
				}
				iStart = strRTFDoc.Find("\\par", iStop+4);
			}
		}
	}

	iNumParagraphs = strParagraphs.GetCount();
	return iNumParagraphs;
}
...
void CAutoRichEditCtrl::SetPlainText(CString sText)
{
	// sets non-formatted text into the control
	EDITSTREAM es;
	es.dwError = 0;
	es.pfnCallback = CBStreamIn;
...
void CAutoRichEditCtrl::SetFontName(CString sFontName)
{
	...
	// Set the font name.
	strcpy_s(cf.szFaceName, sizeof(cf.szFaceName), sFontName.GetBuffer(0));
